...
DESCRIPTION
       ...
       --show-control-chars
              show nongraphic characters as-is (the default, unless program is 'ls' and output is a ter‐
              minal)
       ...
       -u     with -lt: sort by, and show, access time; with -l: show access time and sort by name; oth‐
              erwise: sort by access time, newest first
       ...
       preted like in date(1).  If FORMAT is FORMAT1<newline>FORMAT2, then FORMAT1 applies to non-recent
       files and FORMAT2 to recent files.  TIME_STYLE prefixed with 'posix-' takes effect  only  outside
